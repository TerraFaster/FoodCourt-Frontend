  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center space-x-1 sm:space-x-2 px-3 sm:px-4 py-2.5 text-white hover:text-yellow-400 transition-all duration-300 rounded-xl hover:bg-white/5"
      >
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 rounded-2xl shadow-2xl z-50 min-w-40 sm:min-w-44 backdrop-blur-sm border border-gray-600/30 overflow-hidden" 
             style={{ backgroundColor: 'rgba(26, 26, 26, 0.95)' }}>
          {languages.map((lang, index) => (
            <button
              key={lang.code}
              onClick={() => handleLanguageChange(lang.code)}
              className={`flex items-center gap-3 w-full text-left px-4 sm:px-5 py-3 text-xs sm:text-sm transition-all duration-300 relative ${
        ...
        <div className="flex items-center justify-between h-14 sm:h-16">
          ...
          <Link href="/" className="flex items-center space-x-2 sm:space-x-3 hover:opacity-80 transition-opacity">
            <div className="w-8 h-8 sm:w-10 sm:h-10 bg-yellow-400 rounded-lg flex items-center justify-center">
              <span className="font-bold text-lg sm:text-xl" style={{ color: '#111111' }}>R</span>
            </div>
